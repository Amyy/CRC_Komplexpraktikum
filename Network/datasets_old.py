...
class InstrumentDataset(data.Dataset):
    ...
    def add_sample(self, path, labels):
        ...
        if self.labeled:
            ...
            image_path = data_pathlib / parts[-3] / parts[-2] / parts[-1]
...
# MODIFIED CODE SNIPPET;
        if self.labeled:
            ...
            for op in ops:
                ...
                for i, row in enumerate(reader):
                    if i % self.modulo == 0:
                        ...
                        # TODO image path
                        path = data_pathlib / opset_nr / op_nr / ("%08d.png" % (int(row[0]) / 25))
            ...
            for op in ops:
                ...
                # TODO insert .csv path (not image path)
                f = open(op + "/Ins.csv", "r")
                ...
                for i, row in enumerate(reader):
                    if i % self.modulo == 0:
                        ...
                        opset_nr = split[len(split) -2] # get opset number out of path: /mnt/g27prist/TCO/TCO-Studenten/wagnerame/CRC_Komplexpraktikum/Annotations/4/57
                        op_nr = split[len(split) - 1]
                        labeled_image_path.append(data_pathlib / opset_nr / op_nr / ("%08d.png" % (int(row[0]) / 25)))
